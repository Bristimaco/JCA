...
class MyMembersController extends Controller
{
    ...
    {
        $members = $request->user()->members()->with(['weightCategory', 'tournamentResults.tournament', 'vouchers'])->orderBy('first_name')->get()
            ->map(function (Member $m) {
                ...
                return [
                    ...
                    'voucher' => $m->activeVoucher() ? [
                        'code' => $m->activeVoucher()->code,
                        'status' => $m->activeVoucher()->status->value,
                        'status_label' => $m->activeVoucher()->status->label(),
                        'expires_at' => $m->activeVoucher()->expires_at->format('d/m/Y'),
                    ] : null,
                ];
            });
