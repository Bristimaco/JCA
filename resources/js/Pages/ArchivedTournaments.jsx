    ...
    return (
        <AppLayout>
            ...
            <div className="mb-6 flex items-center gap-4">
                ...
                <h1 className="text-2xl font-bold text-white tracking-tight">Gearchiveerde Toernooien</h1>
            ...
            {tournaments.length === 0 ? (
                <div className="bg-slate-800 rounded-xl shadow-sm ring-1 ring-slate-700/60 px-6 py-12 text-center text-slate-400">
                ...
                <div className="space-y-4">
                    {tournaments.map(t => (
                        <div key={t.id} className="bg-slate-800 rounded-xl shadow-sm ring-1 ring-slate-700/60 overflow-hidden hover:shadow-md">
                                ...
                                <div className="flex items-start justify-between gap-4">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <Link
                                            ...
                                            className="rounded-lg bg-slate-100 px-3 py-1.5 text-xs font-medium text-slate-300 hover:bg-slate-200"
                                        >
                                            Detail bekijken
                                        </Link>
                                        <button
                                            onClick={() => setExpandedId(expandedId === t.id ? null : t.id)}
                                            className="rounded-lg px-3 py-1.5 text-sm font-medium text-amber-400 hover:bg-amber-900/30"
                                        >
                                    ...
                                    <div className="mt-2 flex flex-wrap gap-2">
                                        {t.attachments.map(att => (
                                            <a key={att.id} href={att.url} target="_blank" rel="noopener noreferrer"
                                                className="inline-flex items-center rounded-lg bg-slate-700/50 px-2.5 py-1 text-xs text-amber-400 hover:underline ring-1 ring-slate-700/60">
                                                {att.original_name}
                                            </a>
                                        ))}
                                    </div>
                            ...
                            {expandedId === t.id && (
                                <div className="border-t border-slate-700/60 px-5 py-4 bg-slate-700/30">
                                    {t.results.length === 0 ? (
