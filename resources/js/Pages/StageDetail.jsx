...
export default function StageDetail({ stage, participantGroups, totalParticipants }) {
    const t = stage;
    ...
    const statusColors = {
        ...
        invitations_sent: 'bg-teal-900/30 text-blue-400',
        ...
        <AppLayout>
            ...
            <div className="grid gap-6 lg:grid-cols-3 mt-4">
                ...
                <div className="lg:col-span-1 space-y-4">
                    <div className="bg-slate-900 rounded-xl shadow-sm ring-1 ring-slate-800/60 border-t-2 border-t-teal-700 p-5">
                        <h2 className="text-[11px] font-semibold text-slate-400 uppercase tracking-wider mb-3">Details</h2>
                        <dl className="space-y-3 text-sm">
                            <div>
                                <dt className="text-slate-400 text-xs">Van</dt>
                                <dd className="font-medium text-white">{formatDate(t.start_date)}</dd>
                            </div>
                            <div>
                                <dt className="text-slate-400 text-xs">Tot</dt>
                                <dd className="font-medium text-white">{formatDate(t.end_date)}</dd>
                            </div>
                            <div>
                    ...
                    {hasMap && (
                        <div className="bg-slate-900 rounded-xl shadow-sm ring-1 ring-slate-800/60 border-t-2 border-t-teal-700 overflow-hidden">
                            <iframe
                                title="Locatie"
                                width="100%"
                                height="220"
                ...
                <div className="lg:col-span-2">
                    <div className="bg-slate-900 rounded-xl shadow-sm ring-1 ring-slate-800/60 border-t-2 border-t-teal-700">
                                ...
                                {participantGroups.map(ageGroup => (
                                    <div key={ageGroup.name} className="px-5 py-4">
                                        <h3 className="text-sm font-semibold text-slate-200 mb-3">
                                            <span className="inline-flex items-center rounded-full bg-teal-900/30 px-2.5 py-0.5 text-xs font-semibold text-teal-300">
                                                {ageGroup.name}
                                            </span>
                                        </h3>
                                            ...
                                            {ageGroup.weights.map(weightGroup => (
                                                <div key={weightGroup.name} className="rounded-xl ring-1 ring-slate-800 bg-slate-700/30 p-3">
                                                    <h4 className="text-xs font-semibold text-teal-400 mb-2">
                                                        <span className="inline-flex items-center rounded-full bg-teal-900/30 px-2 py-0.5">
                                                            {weightGroup.name}
                                                        </span>
                                                    ...
                                                    <ul className="space-y-1">
                                                        {weightGroup.members.map(member => (
                                                            <li key={member.id} className="text-sm text-slate-300">
                                                                {member.name}
                                                            </li>
                                                        ))}
                                                    </ul>
