        ...
        <>
            ...
            {products.length === 0 ? (
                ...
                <div className="lg:hidden divide-y divide-slate-800">
                    {products.map((p) => (
                        editingId === p.id
                            ? <EditMobileCard key={p.id} product={p} categories={categories} onCancel={() => setEditingId(null)} onSuccess={() => setEditingId(null)} />
                            : <ProductMobileCard key={p.id} product={p} categoryName={categoryName(p.bar_category_id)} onEdit={() => setEditingId(p.id)} />
                    ))}
                </div>
                </>
            )}
...
function ProductMobileCard({ product, categoryName, onEdit }) {
    const deleteForm = useForm({});
    ...
    return (
        <div className="px-4 py-3 space-y-2">
            ...
            <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
                ...
                <span className="text-sm font-medium text-emerald-400">€{Number(product.price).toFixed(2)}</span>
                <span>#{product.display_order}</span>
            </div>
...
function EditMobileCard({ product, categories, onCancel, onSuccess }) {
    const form = useForm({
        name: product.name,
        price: product.price,
        is_active: product.is_active,
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        form.patch(`/admin/bar-products/${product.id}`, { preserveScroll: true, onSuccess });
    };

    return (
        <form onSubmit={handleSubmit} className="px-4 py-3 space-y-3 bg-slate-800/30">
            <div className="grid grid-cols-2 gap-3">
                <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Naam</label>
                    <input type="text" value={form.data.name} onChange={(e) => form.setData('name', e.target.value)}
                        className="w-full rounded-md bg-slate-800 border-slate-700 text-white text-sm px-2 py-1 focus:ring-rose-500 focus:border-rose-500" />
                </div>
                <div>
                    <label className="block text-xs font-medium text-slate-400 mb-1">Rubriek</label>
                    <select value={form.data.bar_category_id} onChange={(e) => form.setData('bar_category_id', e.target.value)}
            ...
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-3">
                    <button type="submit" disabled={form.processing} className="text-xs font-medium text-emerald-400 hover:text-emerald-300 disabled:opacity-50">
                        {form.processing ? 'Bezig...' : 'Opslaan'}
                    </button>
                    <button type="button" onClick={onCancel} className="text-xs font-medium text-slate-400 hover:text-slate-300">Annuleren</button>
                </div>
